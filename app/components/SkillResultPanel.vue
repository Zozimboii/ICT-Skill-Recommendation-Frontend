...
const subcatLabels = computed(() =>
  props.result.top_subcategories?.map((x) => x.sub_category_name) ?? []
)
const subcatValues = computed(() =>
  props.result.top_subcategories?.map((x) => x.count) ?? []
)

const relatedLabels = computed(() =>
  props.result.related_skills?.map((x) => x.skill_name) ?? []
)
const relatedValues = computed(() =>
  props.result.related_skills?.map((x) => x.count) ?? []
)
</script>
