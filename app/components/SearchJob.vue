...
const fetchAllJobCounts = async () => {
  ...
  try {
    ...
    error.value =
      err?.data?.detail ||
      (err?.message ? `Failed to fetch job counts: ${err.message}` : 'Failed to fetch job counts')
   ...
   <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
    <div class="flex items-start justify-between gap-3">
      <div>
        ...
        <div class="mt-1 text-sm text-slate-600">
          พิมพ์ชื่อ sub-category เพื่อดูจำนวนงาน + หมวดที่เกี่ยวข้อง
        </div>
      </div>
    </div>
    ...
    <div class="mt-4 flex flex-col gap-2 sm:flex-row">
      <input
        ...
        class="w-full flex-1 rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none
    ...
    <!-- Loading -->
    <div v-if="isLoading" class="mt-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
      Loading job counts...
    </div>
    ...
    <div v-else-if="error" class="mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
      ...
      <button
        @click="fetchAllJobCounts"
        class="ml-2 underline decoration-red-300 underline-offset-2 hover:opacity-80"
      >
        Retry
      </button>
    </div>
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div v-if="result.top_categories?.length" class="space-y-3">
          ...
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div
              v-for="cat in result.top_categories"
              :key="`${cat.main_category_id}-${cat.sub_category_id}`"
            ...
            >
              ...
              <div class="mt-3">
                <span class="inline-flex items-center rounded-lg bg-blue-600 px-2.5 py-1 text-sm font-semibold text-white">
                  {{ cat.job_count }} jobs
                </span>
              </div>
            </div>
        ...
        <div v-if="result.related_sub_categories?.length" class="space-y-3">
          ...
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div
              v-for="rel in result.related_sub_categories"
              :key="rel.sub_category_id"
              class="rounded-xl border border-slate-200 bg-slate-50 p-4"
            >
              <div class="font-semibold text-slate-900">{{ rel.sub_category_name }}</div>
              <div class="mt-3">
                <span class="inline-flex items-center rounded-lg bg-emerald-600 px-2.5 py-1 text-sm font-semibold text-white">
                  {{ rel.job_count }} jobs
                </span>
              </div>
            </div>
    ...
    <div v-else class="mt-4 text-sm text-slate-500">
      เริ่มจากพิมพ์ sub-category แล้วกด Refresh หรือพิมพ์แล้วระบบจะค้นหาให้
    </div>
  </div>
</template>
