...
const askAdvisor = async () => {
  ...
  try {
    ...
    // useFetch error บางทีอยู่ใน e.data / e.message
    ...
    <div v-if="result" class="mt-4 space-y-2">
      ...
      <div class="text-sm">
        ...
        <div class="mt-2 flex flex-wrap gap-2">
          <button
            v-for="s in result.suggested_skills"
            :key="s"
            @click="emit('quickSearch', s)"
      ...
      <div v-if="result.trend_preview" class="text-sm text-slate-700">
        ...
        สกิล <span class="font-semibold">{{ result.trend_preview.skill_name }}</span>
        พบใน <span class="font-semibold">{{ result.trend_preview.job_count }}</span> งาน
      </div>
    </div>
  </div>
</template> -->
