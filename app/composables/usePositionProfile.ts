...
export function usePositionProfile() {
    async function getSkills(positionId: string): Promise<PositionSkillsResponse | null> {
        const id = positionId.trim();
        if (!id) return null;

        const { data, error } = await useApiFetch<PositionSkillsResponse>(`${BASE_URL}/${id}/skills`, { method: 'GET' });
